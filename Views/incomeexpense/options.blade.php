    ...
    <script type="text/javascript">
      $(function () {
        $('#btn-run-report').click(function () {
          ...
          $.post("{{ route('addons.reports.incomeexpensereport.validate') }}", {
            from_date: from_date,
            to_date: to_date,
            ...
            if (output_type == 'preview') {
              $('#preview').show();
              $('#preview-results').attr('src', "{{ route('addons.reports.incomeexpensereport.html') }}" + query_string);
            }
            else if (output_type == 'pdf') {
              window.location.href = "{{ route('addons.reports.incomeexpensereport.pdf') }}" + query_string;
            }
...
@section('content')
    <section class="content-header">
        <h1 class="pull-left">{{ trans('IncomeExpenseReport::common.title') }}</h1>
